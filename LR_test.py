...
lr_test_pred_prob = lr_model.predict_proba(X_test_selected)[:, 1]
...
roc_auc = roc_auc_score(y_test, lr_test_pred_prob)
...
brier_score = brier_score_loss(y_test, lr_test_pred_prob)
print("Brier Score:", brier_score)

# Save the predicted probabilities for further analysis
joblib.dump(lr_test_pred_prob, '../test_models/lr_test_probs.joblib')

# Convert y_test and lr_test_pred to numpy arrays if they are not already
y_test = np.array(y_test)
lr_test_pred = np.array(lr_test_pred)
...
accuracy_ci = bootstrap_metric(y_test, lr_test_pred, accuracy_score)
precision_ci = bootstrap_metric(y_test, lr_test_pred, precision_score)
recall_ci = bootstrap_metric(y_test, lr_test_pred, recall_score)
f1_ci = bootstrap_metric(y_test, lr_test_pred, f1_score)
roc_auc_ci = bootstrap_metric(y_test, lr_test_pred_prob, roc_auc_score, y_pred_proba=lr_test_pred_prob)
specificity_ci = bootstrap_metric(y_test, lr_test_pred, specificity_score)
